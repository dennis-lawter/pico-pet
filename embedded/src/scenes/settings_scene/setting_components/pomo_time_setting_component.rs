...
impl Default for PomoTimeSettingComponent {
    fn default() -> Self {
        ...
        let setting = nvm.settings.get_setting(SettingType::PomodoroMinutes);
...
impl SettingComponentTrait for PomoTimeSettingComponent {
    ...
    fn input(&mut self) {
        ...
        if check_if_confirming() {
            ...
            nvm.settings
                .set_value(SettingType::PomodoroMinutes, self.initial_value);
            self.will_be_deselected = true;
        } else {
            adjust_setting(&mut self.setting);
            nvm.settings
                .set_value(SettingType::PomodoroMinutes, self.setting.get_value());
        }
    }
