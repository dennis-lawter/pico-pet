...
pub struct GamePlayState<'a> {
    ...
    frame_count: usize,
    brightness: Setting,
    key_repeat_slowdown_timer: usize,
    playing_sound: bool,
}

impl State<'_> for GamePlayState<'static> {
    fn enter_state(system_ptr: *mut System) -> AppState<'static> {
        ...
        let brightness = Setting::new_max();
        let frame_count = 0;

        let key_repeat_slowdown_timer = 0;
        let playing_sound = false;

        let return_value = AppState::GamePlay(GamePlayState {
            system_ptr,
            ferris,
            corro,
            frame_count,
            brightness,
            key_repeat_slowdown_timer,
            playing_sound,
        });

        render::flood(0b000_000_00);
        render::draw(unsafe { &mut (*system_ptr).display });
    ...
    fn input(&mut self) {
        unsafe {
            if (*self.system_ptr).key0_pressed() {
                self.ferris.x -= 1;
            }
            if (*self.system_ptr).key1_pressed() {
                self.ferris.y += 1;
            }
            if (*self.system_ptr).key2_pressed() {
                self.ferris.y -= 1;
            }
            if (*self.system_ptr).key3_pressed() {
                self.ferris.x += 1;
            }
        }
    }
    ...
    fn draw(&mut self) {
        self.corro.draw(0);
        self.ferris.draw((self.frame_count / 20) % 2);

        unsafe {
            (*self.system_ptr).set_backlight(&self.brightness);
            render::draw(&mut (*self.system_ptr).display);
        }
    }
